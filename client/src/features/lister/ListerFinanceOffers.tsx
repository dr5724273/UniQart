  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      <Card>
        ...
        <form className="mt-4 grid gap-3" onSubmit={(e) => void createOffer(e)}>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <div className="mb-1 text-xs font-semibold text-slate-600">Total Lending Amount</div>
              <Input value={totalAmount} onChange={(e) => setTotalAmount(e.target.value)} required />
            </div>
            <div>
              <div className="mb-1 text-xs font-semibold text-slate-600">Interest Rate (%)</div>
              <Input value={interestRate} onChange={(e) => setInterestRate(e.target.value)} required />
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <div className="mb-1 text-xs font-semibold text-slate-600">Min Loan</div>
              <Input value={minLoan} onChange={(e) => setMinLoan(e.target.value)} required />
            </div>
            <div>
              <div className="mb-1 text-xs font-semibold text-slate-600">Max Loan</div>
              <Input value={maxLoan} onChange={(e) => setMaxLoan(e.target.value)} required />
            </div>
          </div>

          <div>
            <div className="mb-1 text-xs font-semibold text-slate-600">Loan Duration Options</div>
          ...
          <div>
            <div className="mb-1 text-xs font-semibold text-slate-600">Terms & Conditions</div>
      ...
      <Card>
        ...
        <div className="mt-4 grid gap-3">
          ...
          {!loading && items.length === 0 ? <div className="text-sm text-slate-600">No offers yet.</div> : null}
        </div>
      </Card>
    </div>
  );
